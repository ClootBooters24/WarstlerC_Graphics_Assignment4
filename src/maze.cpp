...
void readMaze(){
   ...
   map.resize(xLength, vector<wall>(yLength));
   string line;
   getline(file, line);
   for(int i = 0; i < xLength; i++) {
      getline(file, line);
      for(int j = 0; j < yLength; j++) {
         char wallType = line[j];
         
         switch(wallType) {
            case 'b':
               map[i][j].type = wallType::brick;
               break;
            case 'w':
               map[i][j].type = wallType::wood;
               break;
            case 'r':
               map[i][j].type = wallType::rock;
               break;
            case ' ':
               map[i][j].type = wallType::empty;
               break;
            default:
               cerr << "Invalid character in maze file" << endl;
               exit(1);
         }
      }
   }
...
void block(float xmin, float ymin, float zmin,
           float xmax, float ymax, float zmax, string name)
{
   // Init texture
   int xdim, ydim;
   unsigned char *texture;
   string texturePath = "textures/" + name + ".jpg";
   init_texture(const_cast<char*>(texturePath.c_str()), texture, xdim, ydim);
...
void drawMaze() {
   ...
   for(int i = 0; i < xLength; i++) {
      for(int j = 0; j < yLength; j++) {
         ...
         switch(map[i][j].type) {
            case wallType::brick:
               block(i, j, 0, i+1, j+1, 1, "brick");
               break;
            case wallType::wood:
               block(i, j, 0, i+1, j+1, 1, "wood");
               break;
            case wallType::rock:
               block(i, j, 0, i+1, j+1, 1, "rock");
               break;
            case wallType::empty:
               break;
         }
...
//---------------------------------------
// Function to draw 3D character
//---------------------------------------
void drawCharacter(float xmin, float ymin, float zmin,
           float xmax, float ymax, float zmax) {

   glColor3f(float(255/255), float(223/255), float(0/255));
...
void display()
{
   ...
   drawMaze();
   // drawCharacter(xStart, yStart, 0, xStart + 1, yStart + 1, 1);
